...
`piggyback` is basically a poor soul's [git LFS](https://git-lfs.github.com/).  GitHub rejects commits containing files larger than 50 Mb.  `git LFS` is not only expensive, it also [breaks GitHub's collaborative model](). (Someone wants to submit a PR with a simple edit to your docs, they cannot fork)
...
`piggyback` is inspired by [datastorr]() from [@richfitz](). While `datastorr` provides your data in the R "data package" model of `storr` (your data lives inside an R package that you install and is automatically loaded into memory), `piggyback` tries to immitate the LFS model where your data files are simply stored as part of your repository.  This impementation uses R wrappers to the GitHub API, but an identical client could be implemented in any language with bindings to the GitHub API. 
