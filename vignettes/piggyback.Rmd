...
Many read functions accept URLs, including `read.csv()`, `arrow::read_parquet()`,
`readr::read_csv()`, `data.table::fread()`, and `jsonlite::fromJSON()`, so reading
in one file can be done efficiently by passing along the output of `pb_download_url()`
```{r}
...
Some functions also accept connections when wrapped by `url()`, e.g. for `readRDS()`:
```{r}
...
We can also use this `url()` approach to pass along authentication for private 
repositories, e.g.
```{r}
