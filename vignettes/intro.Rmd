...
```{r  eval=FALSE}
pb_download_url("data/mtcars.tsv.gz", tag = "v0.0.3") 
```

Or store and read the URL directly into R:

```{r eval=FALSE}
url <- pb_download_url("data/mtcars.tsv.gz", tag = "v0.0.3")
readr::read_tsv(url)
```
...
Similarly, you can download all current data assets of the latest or specified release by using `pb_download` without additional arguments: 
