...
```{r}
library(piggyback)
pb_download("iris2.tsv.gz", repo = "cboettig/piggyback-tests")
```

**Note**: Whenever you are working from a location inside a git repository corresponding to your GitHub repo, you can simply omit the `repo` argument and it will be detected automatically.  

Or a specific version:

```{r}
pb_download("iris2.tsv.gz", repo = "cboettig/piggyback-tests", tag = "v0.0.1")
```
...
pb_upload("mtcars.tsv.gz", repo = "cboettig/piggyback-tests", tag = "v0.0.1")
```

`pb_upload()` will overwrite any existing file by default, unless the timestamp the uploaded version is more recent.  You can toggle these settings with `overwrite=FALSE` and `use_timestamps=FALSE`.  
...
Similarly, you can download all current data assets of the latest or specified release by using `pb_download` without a `file` argument:  

```{r eval=FALSE}
pb_download(repo = "cboettig/piggyback-tests", tag = "v0.0.1", dest = tempfile())
```

(Typically all arguments above could be ommitted and simply call `pb_download()`, which will download all data from the latest tag of the current repository into that repository.)

## Timestamps and caching

By default, files will not be transferred if the timestamp is more recent at the destination (e.g. `pb_download()` will not download a copy if the local version is newer than the uploaded version).  If necessary, you can override this behavior by setting `use_timestamps = FALSE` in `pb_download()` or `pb_upload()`.  
